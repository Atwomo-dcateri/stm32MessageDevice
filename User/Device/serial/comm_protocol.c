...
uint16_t Proto_Pack(const Frame *frame, uint8_t *buffer)
{
    ...
    crc = Proto_CalcCRC16(crc_data, 2 + frame->len);
...
int8_t Proto_Unpack(const uint8_t *buffer, uint16_t len, Frame *frame)
{
    ...
    // 检查长度（HEAD2 + TYPE + LEN + DATA + CRC2 + TAIL）
    uint16_t expected_len = 2 + 1 + 1 + data_len + 2 + 1;
    ...
    uint16_t calc_crc = Proto_CalcCRC16(crc_data, 2 + data_len);
    uint16_t recv_crc = ((uint16_t)buffer[4 + data_len] << 8) | buffer[4 + data_len + 1];
